...
top_subproduct = (
    df.groupby("product_subcategory")["revenue"]
    .sum()
    .reset_index()
...
with row2_left:
    ...
    with st.container(key="top_card"):
        ...
        for i, row in enumerate(top_subproduct.itertuples(), 1):
            prod_name = row.product_subcategory
            rev_label = row.revenue_label
            html_str += f"""
<div class="top-product-item">
    <div class="top-product-rank">{i}</div>
    <div class="top-product-details">
        <div class="top-product-name">{prod_name}</div>
        <div class="top-product-rev">{rev_label}</div>
    </div>
</div>
"""
